<button type="button" onclick="left();" style="display:scroll; position:fixed;margin-left:-115px;bottom:400px;" name="button"><img src="./siteimage/left.png" alt=""></button>

<?php
$chapterleft=$dfchapter-1;
$q9 = "SELECT * FROM chapterdata WHERE chapter_fictionid='$dffictionid' AND chapter_number=$chapterleft";
  ...
  if($chapterprice["chapter_category"]=="무료"){ ?>
    <script type="text/javascript">
       function left(){
               ...
               var chapterdata_re = "<?php echo $chapterleft; ?>";
               var fictiondata_re ="<?php echo $dffictionid; ?>";
               ...
               $.ajax({
                 ...
                 location.href="fictionhome.html?fictionid="+<?php echo $dffictionid; ?>+"&chapter="+chapterdata_re;
...
<button type="button" onclick="right();" style="display:scroll; position:fixed;margin-left:58%;bottom:400px;" name="button"><img src="./siteimage/right.png" alt=""></button>
<?php

$chapterright=$dfchapter+1;

$q10 = "SELECT * FROM chapterdata WHERE chapter_fictionid='$dffictionid' AND chapter_number=$chapterright";
$result10 = $mysqli->query( $q10);
$chapterprice2=$result10->fetch_array();
if(empty($chapterprice2)){ ?>
  <script type="text/javascript">
   function right(){
    alert("다음 화가 없습니다.");
  }
...
<?php }else{
  ...
  if($chapterprice2["chapter_category"]==="무료"){  ?>

    <script type="text/javascript">
       function right(){
               var boardname = "fiction";
               var chapterdata_re = "<?php echo $chapterright; ?>";
               var fictiondata_re ="<?php echo $dffictionid; ?>";
               var udata={"boardname":boardname , "chapterdata_re":chapterdata_re , "fictiondata_re":fictiondata_re };
               $.ajax({
                 url: "boardcount_check.php",
...
<?php }elseif($chapterprice2["chapter_category"]==="유료"){  ?>
  ...
  function right(){
          ...
          var fictiondata_re ="<?php echo $dffictionid; ?>";
          var dfuserid = "<?php echo $dfuserid; ?>";
          var idchapterdata="<?php echo $chapterprice2['idchapterdata']; ?>"
          var udata={"boardname":boardname , "chapterdata_re":chapterdata_re , "fictiondata_re":fictiondata_re, "dfuserid":dfuserid, "idchapterdata":idchapterdata };
          $.ajax({
            url: "buylist_check.php",
            type: "post",
...
<?php
$q4 = "SELECT * FROM chapterdata WHERE chapter_fictionid='$dffictionid' AND chapter_number='$dfchapter'";
$result4 = $mysqli->query( $q4);
$readfictionlist =$result4->fetch_array();

  if(empty($readfictionlist["idchapterdata"])){
    ?>
    <script>
    alert("없는 화입니다.");
...
<?php
  ...
  $readfictionlist["chapter_title"];
  $readfictionlist["chapter_description"];
  $readfictionlist["chapter_authorcomment"];

?>
<?php if($_SESSION['usernickname']==$myfictionlist["fiction_author"]){ ?>
<div class="fictionread_bt">
  <form class="" action="fictionhome.html?fictionid=<?php echo $dffictionid; ?>&writeid=3" method="post">
    <input type="hidden" name="chaptertitle_mo" value="<?php echo $readfictionlist["chapter_title"]; ?>">
    <input type="hidden" name="chapterdescription_mo" value="<?php echo $readfictionlist["chapter_description"]; ?>">
    <input type="hidden" name="chapterauthorcomment_mo" value="<?php echo $readfictionlist["chapter_authorcomment"]; ?>">
    <input type="hidden" name="fictionid_mo" value="<?php echo $dffictionid; ?>">
    <input type="hidden" name="chapternumber_mo" value="<?php echo $dfchapter; ?>">
<input type="hidden" name="idchapterdata_mo" value="<?php echo $readfictionlist["idchapterdata"];; ?>">
    <input type="submit" name="" style="float:right;margin:3px;" class="btn btn-outline-dark" value="글수정">
  </form>
  <form class="" name="chapterdeletefrm" action="chapterdelete_ok.php" method="post">
    <input type="hidden" name="idchapterdata" value="<?php echo $readfictionlist["idchapterdata"]; ?>">
    <input type="hidden" name="fictionid" value="<?php echo $dffictionid; ?>">
    <input type="hidden" name="chapternumber" value="<?php echo $dfchapter ?>">
    <input type="hidden" name="author" value="<?php echo $myfictionlist["fiction_author"]; ?>">
    <input type="button" name="" style="float:right;margin:3px;" value="글삭제" class="btn btn-outline-dark" onclick="chapterdelete();">
    <script type="text/javascript">
...
<div class="fictionread_container">
   ...
   <h4 style="word-break:break-all;text-align:left;padding:10px;border-bottom:1px dashed gray;"><?php echo $readfictionlist["chapter_title"]; ?></h4>

   <p style="white-space:pre-line;text-align:left;padding:10px;border-bottom:1px dashed gray;"><?php echo $readfictionlist["chapter_description"]; ?></p>
   <p style="text-align:left;padding-left:10px;padding-top:10px;">작가의 말</p>
   <p style="word-break:break-all;text-align:left;padding:10px;"><?php echo $readfictionlist["chapter_authorcomment"]; ?></p>
</div>
...
<div class="comment_container">
  <div class="" style="border-bottom:1px solid blue;">
    <h4 style="padding-top:10px;padding-bottom:10px;text-align:left;">Comment '<?php echo $readfictionlist['chapter_commentcount']; ?></h4>
  </div>
  <?php
  ...
  $q5 = "SELECT COUNT(*) as cnt FROM chaptercommentdata WHERE fictiondataid='$dffictionid' AND chapterdataid='$dfchapter' ";
  ...
  $q2 = "SELECT * FROM chaptercommentdata WHERE fictiondataid='$dffictionid' AND chapterdataid='$dfchapter' order by idchaptercommentdata asc limit $limit_start2,$onepage2";
  $result2 = $mysqli->query( $q2);

  while($board2 = $result2->fetch_array())
  { $idchaptercommentdata=$board2['idchaptercommentdata'];
      ...
      <!--  <a href="#" style="float:right;padding-right:10px;" >답댓글</a> -->
        <?php if($_SESSION['usernickname']===$board2['author']){ ?>
          <a href="#" style="float:right;padding-right:10px;" onclick="commentdelete_<?php echo $idchaptercommentdata; ?>();">삭제</a>
           <form class="" name="commentdeletefrm_<?php echo $idchaptercommentdata; ?>" action="chaptercommentdelete_ok.php" method="post">
             <input type="hidden" name="idchaptercommentdata" value="<?php echo $idchaptercommentdata; ?>">
             <input type="hidden" name="commentauthor" value="<?php echo $chaptercommentauthor; ?>">
             <input type="hidden" name="fictionid" value="<?php echo $dffictionid; ?>">
             <input type="hidden" name="chapter" value="<?php echo $dfchapter; ?>">
...
<div class="" id="subpage">
 <!-- 처음 버튼 -->
  ...
  ?>
       <a href="./fictionhome.html?fictionid=<?php echo $dffictionid; ?>&chapter=<?php echo $dfchapter; ?>&page=<?php echo $page; ?>&subpage=1">처음</a>
  ...
  ?>
       <a href="./fictionhome.html?fictionid=<?php echo $dffictionid; ?>&chapter=<?php echo $dfchapter; ?>&page=<?php echo $page; ?>&subpage=<?php echo $prev_block2; ?>">이전</a>
  ...
  ?>
         <a href="./fictionhome.html?fictionid=<?php echo $dffictionid; ?>&chapter=<?php echo $dfchapter; ?>&page=<?php echo $page; ?>&subpage=<?php echo $i; ?>">[<?php echo $i; ?>]</a>
  <?php
       }
     }
  ?>
 <!-- 다음 버튼 -->
  <?php if($block2 >= $total_block2){
    ...
    $next_block2=$block_end2 +1;
     ?>
     <a href="./fictionhome.html?fictionid=<?php echo $dffictionid; ?>&chapter=<?php echo $dfchapter; ?>&page=<?php echo $page; ?>&subpage=<?php echo $next_block2; ?>">다음</a>
   <?php } ?>
  <!-- 마지막 버튼 -->
  <?php if($subpage>=$total_page2){ ?>
    <span>마지막</span>
 <?php }else{ ?>
    <a href="./fictionhome.html?fictionid=<?php echo $dffictionid; ?>&chapter=<?php echo $dfchapter; ?>&page=<?php echo $page; ?>&subpage=<?php echo $total_page2; ?>">마지막</a>
 <?php } ?>
</div>
<!--페이징 끝-->
<?php }else{} ?>
...
<div class="commentwrite_container">
  <form class="" name="commentregisterfrm" action="chaptercommentregister_ok.php" method="post">
...
if($myfictionlist["fiction_pricecategory"]=="무료"){
 ...
 <div class="fictiontable_container" >
   <table class="list-fictiontable">
       ...
       if(is_numeric($page)==false||$page<0||$page>$total_page){
         ?>
         <script type="text/javascript">
           alert("잘못된 접근입니다.");
